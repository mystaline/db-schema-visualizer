...
const outgoingFks = computed(() =>
  schemaStore.foreignKeys.filter(fk => fk.sourceTableId === schemaStore.selectedTableId)
);

// FKs from other tables that reference this table
const incomingFks = computed(() =>
  schemaStore.foreignKeys.filter(fk => fk.targetTableId === schemaStore.selectedTableId)
);
...
// Columns of selected target table (filtered by PK or Unique)
...
<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      ...
      <span class="text-xs text-secondary-400 font-medium">DEFINED: {{ currentTableFks.length }}</span>
    </div>

    <!-- Outgoing FKs (this table → other) -->
    <div class="space-y-2">
      <p class="text-[10px] font-bold text-secondary-500 uppercase tracking-widest">Outgoing</p>
      <p v-if="outgoingFks.length === 0" class="text-[11px] text-secondary-500 italic px-1">
        No foreign keys originating from this table.
      </p>
      <div
        v-for="fk in outgoingFks"
        :key="fk.id"
        class="bg-secondary-900/40 p-4 rounded-xl border border-secondary-800/50 space-y-3 group"
      >
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-2 text-sm">
            <span class="font-mono text-primary-400">{{ getColumnName(fk.sourceTableId, fk.sourceColumnId) }}</span>
            <svg class="w-3 h-3 text-secondary-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14 5l7 7m0 0l-7 7m7-7H3" />
            </svg>
            <span class="text-secondary-400 font-mono">{{ getTableName(fk.targetTableId) }}.</span><span class="font-mono text-success-400">{{ getColumnName(fk.targetTableId, fk.targetColumnId) }}</span>
          </div>
          <button
        ...
        <div class="flex gap-4 text-[10px] font-bold uppercase tracking-wider">
          <div class="flex items-center gap-1.5 px-2 py-1 bg-secondary-950 rounded border border-secondary-800 text-secondary-500">
            ON DELETE: <span class="text-primary-500">{{ fk.onDelete }}</span>
          </div>
          <div class="flex items-center gap-1.5 px-2 py-1 bg-secondary-950 rounded border border-secondary-800 text-secondary-500">
            ON UPDATE: <span class="text-primary-500">{{ fk.onUpdate }}</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="space-y-2">
      <div class="flex items-center gap-2 py-1">
        ...
        <span class="text-[9px] font-bold text-secondary-500 uppercase tracking-[0.2em]">Incoming References</span>
        <div class="h-px flex-1 bg-secondary-600"></div>
      </div>
      <p v-if="incomingFks.length === 0" class="text-[11px] text-secondary-500 italic px-1">
      ...
      >
        <span class="font-mono text-secondary-400">{{ getTableName(fk.sourceTableId) }}.</span><span class="font-mono text-primary-400">{{ getColumnName(fk.sourceTableId, fk.sourceColumnId) }}</span>
        <svg class="w-3 h-3 text-secondary-600 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14 5l7 7m0 0l-7 7m7-7H3" />
        </svg>
        <span class="font-mono text-success-400">{{ getColumnName(fk.targetTableId, fk.targetColumnId) }}</span>
      </div>
    </div>
    ...
    <div v-if="schemaStore.viewMode === 'full'" class="space-y-4">
      <div class="flex items-center gap-2 py-1">
        <div class="h-px flex-1 bg-secondary-600"></div>
        <span class="text-[9px] font-bold text-secondary-500 uppercase tracking-[0.2em]">Establish Link</span>
        <div class="h-px flex-1 bg-secondary-600"></div>
      </div>
      <h5
        class="sr-only"
      ...
      <div class="grid grid-cols-2 gap-3">
        ...
        <div class="space-y-1.5">
          <label class="text-[10px] font-bold text-secondary-300 uppercase ml-1">Source Column</label>
          <select
        ...
        <!-- Target Table -->
        <div class="space-y-1.5">
          <label class="text-[10px] font-bold text-secondary-300 uppercase ml-1">Target Table</label>
          <select
      ...
      <div class="space-y-1.5">
        <label class="text-[10px] font-bold text-secondary-300 uppercase ml-1">Target Reference (PK/Unique)</label>
        <select
      ...
      <div class="grid grid-cols-2 gap-3">
        <div class="space-y-1.5">
          <label class="text-[10px] font-bold text-secondary-300 uppercase ml-1">On Delete</label>
          <select
        ...
        <div class="space-y-1.5">
          <label class="text-[10px] font-bold text-secondary-300 uppercase ml-1">On Update</label>
          <select
