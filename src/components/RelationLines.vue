...
const tableCoordsMap = computed(() => {
  const map = new Map<
    string,
    { x: number; y: number; columns: { id: string; name: string }[] }
  >();
...
const relations = computed(() => {
  ...
  return schemaStore.foreignKeys
    .map((fk) => {
      const source =
        active?.id === fk.sourceTableId ? active : coords.get(fk.sourceTableId);
      const target =
        active?.id === fk.targetTableId ? active : coords.get(fk.targetTableId);
      ...
      const y1 =
        source.y + HEADER_HEIGHT + sourceColIndex * ROW_HEIGHT + ROW_HEIGHT / 2;
      const y2 =
        target.y + HEADER_HEIGHT + targetColIndex * ROW_HEIGHT + ROW_HEIGHT / 2;
      ...
      return {
        ...
        isSelected:
          schemaStore.selectedTableId === fk.sourceTableId ||
          schemaStore.selectedTableId === fk.targetTableId,
