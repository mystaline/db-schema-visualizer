...
watch(
  ...
  () => {
    if (schemaStore.selectedTable) {
      const generatedName = schemaStore.getIndexName(
...
const toggleColumn = async (id: string) => {
  const idx = newIndex.value.parts.findIndex(
    (p) => p.type === "column" && p.value === id,
  );
  if (idx === -1) {
...
const injectColumn = async (colName: string) => {
  ...
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      ...
      <span class="text-xs text-secondary-400 font-medium"
        >DEFINED: {{ schemaStore.selectedTable?.indexes.length }}</span
      >
    </div>
      ...
      >
        <div class="flex items-start justify-between gap-2 mb-2">
          <div class="flex flex-wrap items-start gap-2 flex-1 min-w-0">
            <span
              class="text-sm font-bold text-secondary-200 break-words w-full"
              >{{ idx.name }}</span
            >
            <span
          ...
          <span
            ...
            :class="
              part.type === 'column'
                ...
                : 'bg-primary-500/10 border border-primary-500/20 text-primary-400 italic'
            "
          >
            {{
            ...
            <span
              v-if="part.order === 'DESC'"
              class="text-[8px] font-bold text-primary-400"
              >DESC</span
            >
          </span>
        </div>
    ...
    <div v-if="schemaStore.viewMode === 'full'" class="space-y-4">
      ...
      <div class="space-y-1.5">
        ...
        <input
          ...
          @input="
            (e) => {
              let v = (e.target as HTMLInputElement).value.replace(
                /[^a-zA-Z0-9_]/g,
                '',
              );
              if (v && /^\d/.test(v)) v = '_' + v;
          ...
          "
        />
        <p v-if="indexNameError" class="text-[10px] text-danger-500 ml-1">
          {{ indexNameError }}
        </p>
      </div>
      ...
      <div class="space-y-1.5">
        <div class="flex items-center justify-between">
          <label class="text-[10px] font-bold text-secondary-300 uppercase ml-1"
            >Index Parts (Columns/Exprs)</label
          >
          <label
            class="text-[10px] text-secondary-500 ml-1 flex items-center gap-1.5"
          >
            <input
        ...
        <!-- Selected Parts -->
        <div
          class="space-y-1.5 p-3 bg-secondary-950 border border-secondary-800 rounded-lg min-h-[50px] flex flex-col gap-2"
        >
          <div
            v-if="!newIndex.parts.length"
            class="text-[10px] text-secondary-600 italic"
          >
            Select columns or add expressions below...
          </div>
          <div
          ...
          >
            <div class="flex items-center gap-2 overflow-hidden">
              <span
                class="text-[9px] font-bold text-secondary-500 opacity-50"
                >{{ i + 1 }}</span
              >
              <span
              ...
              <span
                class="text-[10px] font-mono text-secondary-200 truncate"
                :title="
                  part.type === 'column'
                    ? getColumnName(part.value)
                    : part.value
                "
              >
                {{
        ...
        <div class="flex flex-wrap gap-1.5 pt-1">
          <button
            ...
            :class="
              newIndex.parts.some(
                (p) => p.type === 'column' && p.value === col.id,
              )
      ...
      <div class="space-y-1.5">
        ...
        <div class="flex flex-wrap gap-1 mt-1 opacity-70">
          <span class="text-[9px] text-secondary-500 flex items-center mr-1"
            >Tap to inject:</span
          >
          <button
            v-for="col in schemaStore.selectedTable?.columns"
      ...
      <button
        :disabled="
          newIndex.parts.length === 0 || !newIndex.name || !!indexNameError
        "
        class="w-full flex items-center justify-center gap-2 py-2.5 border border-secondary-700 bg-transparent hover:bg-secondary-800 rounded-lg text-secondary-400 hover:text-secondary-100 text-xs font-semibold transition-all disabled:opacity-20 group active:scale-95 mt-2"
        @click="addIndex"
      >
        <svg
