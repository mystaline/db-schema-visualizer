...
const addConstraint = () => {
  if (
    ...
    !newConstraint.value.expression.trim() ||
    !newConstraint.value.name ||
    constraintNameError.value
  )
    return;
  ...
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      ...
      <span class="text-xs text-secondary-400 font-medium"
        >DEFINED: {{ schemaStore.selectedTable?.checkConstraints.length }}</span
      >
    </div>
    ...
    <div class="space-y-2">
      ...
      >
        ...
        <div
          class="bg-secondary-950/50 p-2 rounded-lg border border-secondary-800/30"
        >
          <p
            class="text-[10px] font-mono text-primary-400 italic leading-relaxed"
          >
    ...
    <div v-if="schemaStore.viewMode === 'full'" class="space-y-4">
      ...
      <div class="space-y-1.5">
        ...
        <input
          ...
          "
        />
        <p v-if="constraintNameError" class="text-[10px] text-danger-500 ml-1">
          {{ constraintNameError }}
        </p>
      </div>
