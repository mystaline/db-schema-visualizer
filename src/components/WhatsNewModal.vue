...
const badgeColor: Record<string, string> = {
  ...
  fix: "bg-success-500/15 text-success-400 border-success-500/30",
};

const typeMeta: Record<string, { label: string; color: string }> = {
  feature:     { label: "New",      color: "text-primary-400 bg-primary-500/10 border-primary-500/20" },
  improvement: { label: "Improved", color: "text-info-400 bg-info-500/10 border-info-500/20" },
  fix:         { label: "Fix",      color: "text-success-400 bg-success-500/10 border-success-500/20" },
};
      ...
      >
        ...
        >
          <div
            v-if="isOpen"
            ref="modalRef"
            class="relative w-full max-w-2xl max-h-[88vh] flex flex-col rounded-3xl overflow-hidden shadow-[0_0_100px_rgba(0,0,0,0.9)] border border-secondary-800 bg-secondary-900"
          >
            <!-- Accent bar -->
            <div class="h-1 w-full bg-linear-to-r from-primary-500 via-info-400 to-primary-700 shrink-0" />

            <!-- Header -->
            <div class="px-8 pt-7 pb-5 shrink-0 relative overflow-hidden">
              <div class="absolute -top-10 -right-10 w-48 h-48 bg-primary-500/10 rounded-full blur-3xl pointer-events-none" />
              <div class="absolute -top-10 left-10 w-32 h-32 bg-info-500/8 rounded-full blur-3xl pointer-events-none" />

              <div class="flex items-center justify-between relative">
                <div class="flex items-center gap-4">
                  <div class="w-11 h-11 rounded-2xl bg-linear-to-br from-primary-500/20 to-info-500/20 border border-primary-500/30 flex items-center justify-center shrink-0">
                  ...
                  <div>
                    ...
                    <p class="text-[10px] font-bold text-secondary-500 uppercase tracking-[0.18em] mt-1">
            ...
            <!-- Accordion body -->
            <div class="flex-1 overflow-y-auto custom-scrollbar px-6 pb-6 space-y-2">
              <div
                ...
                :class="
                  openVersion === entry.version
                    ? 'border-secondary-600 bg-secondary-800/40'
              ...
              >
                ...
                >
                  <div class="flex items-center gap-3 min-w-0">
                    ...
                    <!-- Version badge -->
                    <span class="text-sm font-black font-mono text-secondary-200 shrink-0">
                      v{{ entry.version }}
                    </span>
                    ...
                    <!-- Date -->
                    <span class="text-[10px] text-secondary-600 font-medium truncate">
                      {{ entry.date }}
                    </span>
                  </div>
                ...
                >
                  <div v-if="openVersion === entry.version" class="px-5 pb-4 space-y-2 border-t border-secondary-700/50">
                    <div
                    ...
                    >
                      ...
                      <span class="text-sm leading-relaxed"
                        :class="idx === 0 ? 'text-secondary-200' : 'text-secondary-400'"
                      >
                        {{ item.text }}
                      </span>
                    </div>
            ...
            <!-- Footer -->
            <div class="px-8 py-5 border-t border-secondary-800 bg-secondary-950/40 shrink-0 flex items-center justify-between">
              <span class="text-xs text-secondary-600 font-mono">SchemaVis · v{{ APP_VERSION }}</span>
              <button
