...
import { pgTypeToPrisma } from "../shared/pgTypeMap";
...
export function buildPrismaSchema(tables: SchemaTable[], foreignKeys: ForeignKey[]): PrismaResult {
  ...
  for (const t of tables) {
    ...
    if (pkCols.length === 0) {
      warnings.push(`SchemaTable "${t.name}" has no primary key — Prisma requires @id or @@id`);
    }
